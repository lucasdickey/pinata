...
const PINATA_INSPECT = (context) => {
  const limits = context.limits;
  const clean = (value, max) => {
    if (typeof value !== "string") return "";
    let out = "";
    for (let i = 0; i < value.length && out.length < max * 2; i += 1) {
      const code = value.charCodeAt(i);
      if (code === 0x2028 || code === 0x2029 || (code >= 0x202a && code <= 0x202e)) continue;
  ...
  const visible = (el) => {
    ...
    if (details && !details.open && el !== details && el.tagName !== "SUMMARY") return false;
    return true;
  };
  ...
  for (let i = 0; i < nodes.length; i += 1) {
    ...
    const classes = [];
    if (el.classList) {
      for (let c = 0; c < el.classList.length && c < limits.maxClasses; c += 1) {
        classes.push(clean(el.classList[c], 64));
      }
    }
    const text = clean(el.textContent || "", limits.textMaxChars);
    const name = clean(
      el.getAttribute("aria-label") || el.getAttribute("alt") || el.getAttribute("title") || text,
      limits.nameMaxChars
    );
    candidates.push({
      id: "",
  ...
  // Even-stride sampling keeps top, middle, and bottom coverage when a page
  // has more candidates than the budget allows.
  const sample = (list, keep) => {
    if (list.length <= keep) return list.slice();
    const out = [];
    for (let i = 0; i < keep; i += 1) out.push(list[Math.floor((i * list.length) / keep)]);
    return out;
  };
  const measure = (list) =>
    new TextEncoder().encode(
      JSON.stringify({ schemaVersion: limits.manifestSchemaVersion, truncated: false, elements: list })
    ).length;

  let keep = Math.min(candidates.length, limits.manifestMaxElements);
  let elements = sample(candidates, keep);
  let truncated = keep < candidates.length;
  while (elements.length > 1 && measure(elements) > limits.manifestMaxBytes) {
    keep = Math.floor(elements.length * 0.8);
    elements = sample(candidates, keep);
    truncated = true;
  }
  for (let i = 0; i < elements.length; i += 1) elements[i].id = "e" + (i + 1);
