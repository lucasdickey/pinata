  ...
  "decisions": [
    ...
    },
    {
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "test/fixtures/capture/remote-network-v1.html",
          "caption": "Survivable 14-probe matrix with self-canceling probes and runtime-assembled leak marker"
        },
        {
          "type": "file",
          "path": "test/fixtures/capture/remote-network-hard-v1.html",
          "caption": "Session-fatal literal destinations; expected outcome is a bounded safe failure with zero artifacts"
        },
        {
          "type": "file",
          "path": "test/integration/browserless-network-safety.integration.test.ts",
    ...
    },
    {
      ...
      "date": "2026-09-09",
      "phase": "build",
      "title": "Editor project entry is one explicit four-state list machine with a single-flight retry, and e2e run cleanup lives in teardown",
      "origin": "agent-autonomous",
      "status": "accepted",
      "problem": "VAL-AUTH-008/009 require the authenticated project list to have distinct, announced loading, empty, populated, and failure states, with a failure retry that cannot multiply reads and an empty state offering exactly one create action. Separately, two validation sessions leaked run-scoped rows into the real Turso database because run cleanup lived in a trailing Playwright test, which an aborted or failed run never reaches.",
      ...
      "alternatives": [
        {
          "option": "Keep cleanup as a final verification test",
          "why_not": "An aborted or failed run skips trailing tests, and this already leaked two projects and a dozen idempotency keys into the real database; teardown hooks run on abort, trailing tests do not."
        },
        {
          "option": "Let the retry button re-click freely and dedupe server-side",
          "why_not": "The list read is idempotent, but the contract asks for one request per retry intent; disabling the in-flight control makes the single-read guarantee a client-side fact provable by request counting instead of an inference."
        },
        {
          "option": "Route the empty state to a separate /projects/new page",
          "why_not": "VAL-AUTH-009 requires entry without manual route entry and no resubmission on Back/Forward; an in-place form on the single landing route satisfies both with no history entries at all."
        }
      ],
      ...
      "artifacts": [
        {
          ...
          "caption": "Four-state project list with single-flight retry and in-region create control"
        },
        {
          "type": "file",
          "path": "test/editor-home-entry.test.tsx",
          "caption": "Component proof: distinct announced states, one-read retry, form retention across background failure"
        },
        {
          "type": "file",
      ...
      "artifacts": [
        {
          ...
          "caption": "Single darkened --accent token with the AA floor documented at the point of definition"
        },
        {
          "type": "file",
          "path": "src/components/decisions-catalog.tsx",
          "caption": "External-host suffix on artifact links, h2/h3 heading order, per-decision landmark labels"
        },
        {
          "type": "file",
          "path": "test/visual-tokens.test.ts",
          "caption": "WCAG luminance checks locking --accent at >=4.5:1 on both backgrounds and as badge fill"
        },
        {
          "type": "file",
          "path": "test/requirements-decisions.test.tsx",
          "caption": "Host-suffix, heading-order, and landmark-uniqueness assertions against the real decision log"
        }
      ]
    },
    {
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "src/lib/markdown.ts",
          "caption": "Fenced-code <pre> and .table-scroll render as tabindex=0 role=group named scroll regions"
        },
        {
          "type": "file",
          "path": "e2e/requirements-a11y.spec.ts",
          "caption": "axe wcag2a/2aa sweep at desktop and 390px on all five /reqs routes plus real keyboard-scroll proof"
        },
        {
          "type": "file",
          "path": "test/requirements-markdown.test.ts",
          "caption": "Renderer contract tests pinning the focusable, named scroll-region markup"
        },
        {
          "type": "file",
          "path": "scripts/lib/approved-deps.mjs",
          "caption": "Approved dev-dependency allowlist extended with @axe-core/playwright"
        }
      ]
    },
    {
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "src/lib/capture-dispatch.ts",
          "caption": "Pure driver policy (targets, capped batching, re-drive deferral) plus the scoped-route dispatch helper"
        },
        {
          "type": "file",
          "path": "src/components/editor-home.tsx",
          "caption": "Driver effect wired next to the polling effect on every ready hierarchy read"
        },
        {
          "type": "file",
          "path": "test/editor-home-dispatch.test.tsx",
          "caption": "Wired driver proof: cap respected, quota re-drive, terminal outcomes never loop, conflicts never storm"
        },
        {
          "type": "file",
          "path": "e2e/capture-driver.spec.ts",
          "caption": "Real-provider proof: four attempts reach ready with zero manual dispatch calls and at most two in flight"
        }
      ],
      "key": true
    },
    {
      ...
      "transcript": {
        "request": "\"Trim scope to shorten critical path\" — \"remove Chickpea deployment, and variant/retry integraiton matrix\""
      },
      "artifacts": [
        {
          "type": "file",
          "path": "docs/MILESTONES.md",
          "caption": "Milestone 1 bullets corrected: no deployment, local-only Chickpea validation; milestone 2 now owns the first production deployment and the real Chickpea project"
        }
      ],
      "supersedes": null,
      "superseded_by": null
    ...
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "Pin geometry lives in a pure adapter (canonical tip plus zoom-aware hit box); annotation children carry no React Flow parent extent, and the drag grab offset is captured once per gesture",
      "origin": "agent-autonomous",
      "status": "accepted",
      "problem": "Bringing draft pins onto the canvas needed answers to three coupled questions: what the canonical geometry of a pin is (so persistence later stores one unambiguous fact), how a pin stays grabbable at every zoom without its anchor drifting, and who clamps drags at the frame edge. End-to-end testing then exposed that React Flow's parent-extent clamp rewrites emitted drag positions at the frame edge, which stranded the re-derived tip one grab offset inside the boundary (a tip dragged to the corner settled at natural (1.5, 3) instead of (0, 0)), and that React Flow re-emits the final drag position on pointer-up, which advanced a frame-clamped tip with no pointer movement at all.",
      "decision": "The canonical pin fact is the tip in screenshot-natural CSS pixels. A pure adapter (src/lib/canvas/geometry.ts) owns all conversions: an inclusive document clamp, and a hit box whose on-screen edge never drops below the shared 24px minimum target (it grows in natural units as zoom deepens) while the tip stays recoverable exactly as box plus recorded offsets. Draft pin nodes are React Flow children of the screenshot frame WITHOUT extent: \"parent\" — the adapter is the single clamping authority. The drag grab offset is captured once at drag start and held for the whole gesture. Placement taps are disambiguated from drags by a 6px screen slop, and the draft is transient local UI state (one per plane, Escape clears, never persisted).",
      "alternatives": [
        {
          "option": "Keep extent: \"parent\" and compensate for the clamp in the drag handler",
          "why_not": "The extent clamp hides how far past the edge the pointer is, so no handler-side compensation can recover the canonical boundary tip; it can only guess. Verified by tracing emitted positions end-to-end."
        },
        {
          "option": "Re-derive the grab offset from the current box on every position change",
          "why_not": "When the box is frame-clamped but the tip is not at the boundary, each re-derivation shifts the offsets, and React Flow's drag-end position re-emission then moves the tip with no pointer movement. Measured: the tip advanced on pointer-up."
        },
        {
          "option": "Make the hit box a fixed natural-pixel size at every zoom",
          "why_not": "At 8x a fixed natural box shrinks far below the 24px shared minimum target and pins become ungrabbable exactly when precision matters; a fixed screen box would dwarf the document at overview zoom."
        },
        {
          "option": "Persist the draft tip optimistically on placement",
          "why_not": "D051 descoped the roadmap to pins-first and the annotation API does not exist yet; persistence with numbering is the next milestone feature, and an optimistic write now would ship an unreviewed server contract."
        }
      ],
      "rationale": "These are technical choices inside the already-approved canvas-and-pins direction (D051, D055): they change no user-visible scope, add no dependency or server contract, and were forced by measured end-to-end behavior, so they were safe to decide without surfacing. A single pure adapter keeps every screen-to-natural conversion in one tested boundary, which is what makes the one-natural-pixel inverse-transform contract provable at 1x and 8x.",
      "consequences": [
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "src/lib/canvas/geometry.ts",
          "caption": "The pure natural-pixel coordinate adapter: clamps, hit boxes, grab-offset drag math."
        },
        {
          "type": "file",
          "path": "e2e/canvas-interactions.spec.ts",
          "caption": "Mouse and CDP-touch e2e proving placement, grab-offset drag, corner clamps, and plane isolation at 1x and 8x."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "Capture-driver e2e asserts the server fence (one claiming answer, fenced redrives) instead of a fixed per-attempt dispatch count",
      "origin": "agent-autonomous",
      "status": "accepted",
      "problem": "Under full-suite parallel load the capture-driver e2e flaked on two counting assertions that encoded wrong premises: an in-flight watermark measured at requestfinished (but the client caps by promise settlement, and a 2xx fetch resolves at response headers, so body-delivery lag read as a phantom cap violation), and a hard bound of two dispatches per attempt (but a hierarchy read that still shows a just-claimed row as pending legitimately re-drives the attempt, and the reload race can hide the claiming 2xx from the page entirely). The durable contract — the server fence claims an attempt at most once and every later dispatch is fenced — was never actually broken.",
      "decision": "The test now counts a dispatch as in flight only until the server answers (response event), and per attempt asserts the real invariant: at most one 2xx claiming answer, every other answer fenced (409 conflict or 429 quota), and a small absolute dispatch bound (8) so \"never an unbounded retry\" stays explicit.",
      "alternatives": [
        {
          "option": "Keep the two-per-attempt count bound and re-run until green",
          "why_not": "The premise is false once hierarchy reads can lag the claim transition; the suite would stay flaky and every flake would erode trust in the gate."
        },
        {
          "option": "Serialize the e2e suite to one worker",
          "why_not": "The two-worker bound is a deliberate contract choice; slowing the whole gate to protect one test's wrong premise trades away CI time for nothing learned."
        },
        {
          "option": "Change the driver to never re-drive a conflict",
          "why_not": "A conflict can also mean another client claimed and then died; the deferred re-drive is how the attempt eventually gets driven again. The behavior is sound; the test premise was wrong."
        }
      ],
      "rationale": "Test-only change that strengthens what is actually proven (the fence, the cap, the stand-down) while dropping a count bound whose premise read-after-write timing invalidates. Safe to decide unilaterally: no product code, route, or schema changes, and the new assertions fail loudly if the fence ever really breaks.",
      "consequences": [
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "e2e/capture-driver.spec.ts",
          "caption": "Fence-based per-attempt assertions and the response-time in-flight watermark."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "The canvas documents its own interactions on the page: pan, zoom, pin drop, comment, save, cancel, and opening a saved pin are all taught by persistent on-page instructions",
      "origin": "user-directed",
      "status": "accepted",
      "problem": "A first-time user opening a capture had no way to discover the pin workflow: the canvas supported pan, zoom, an explicit Place pin mode, drafts, and saving, but nothing on the page said so. The reviewer directive was explicit that the page itself must teach the workflow.",
      "decision": "The workspace carries a persistent plain-language hint above the canvas that names every real interaction of this build: drag to pan, scroll or pinch to zoom, the camera buttons, Place pin mode with click/tap to drop a pin, writing a comment and pressing Save pin, Escape or Cancel discarding a draft, clicking a saved pin or its Pins-list entry to read its comment, and dragging a pin to move it. The empty pins panel repeats the discovery path, and the hint names no affordance that does not exist (no dead 'coming soon' features).",
      "alternatives": [
        {
          "option": "A one-time onboarding tooltip or tour",
          "why_not": "Dismissable UI fails the directive: the instructions must be persistent so the page stays self-documenting on every visit, and a tour is another dismissible surface to maintain."
        },
        {
          "option": "Rely on the validation contract and README to document interactions",
          "why_not": "The directive was precisely that the page itself must teach the workflow; external documents are not the page."
        }
      ],
      "rationale": "Direct execution of the verbatim request. The hint is intentionally exhaustive about the current build and nothing beyond it, so it can never drift into promising dead affordances.",
      "consequences": [
      ...
      "transcript": {
        "request": "add instrucitons on teh page itself to make it self-documented"
      },
      "artifacts": [
        {
          "type": "file",
          "path": "src/components/project-workspace.tsx",
          "caption": "The persistent workspace hint and the empty-pins discovery copy."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "Pin persistence: server-assigned monotonic numbering inside the idempotency transaction, one create per saved draft, one revisioned write per drag, and authoritative reloads after failure",
      "origin": "agent-autonomous",
      "status": "accepted",
      "problem": "Persisting pins raised four coupled protocol questions: who assigns pin numbers (and how cancelled or failed drafts must never consume one), how a retried or double-submitted save stays exactly-once, how a drag move commits without write amplification, and what the UI shows when a write fails.",
      "decision": "The server assigns numbers inside the create transaction as max(number)+1 over ALL rows of the capture including tombstones, backstopped by the (capture_id, number) unique index with a bounded collision retry, so deleted or failed numbers are never reused and drafts never reserve one. The annotations create route is idempotency-record-first: an exact replay returns the original record, a key reused with a different payload conflicts, and a failed validation consumes nothing. The client holds one idempotency key per draft intent, POSTs once per Save, and re-reads the capture's pin list after success rather than patching local state. A pin drag is local-only movement committed as exactly one revisioned PATCH at drag end (capture-bound, bounds-validated, revision-bumped); a failed move shows a bounded error and reloads the authoritative list so the pin snaps back. Camera, selection, and cancel paths issue zero writes.",
      "alternatives": [
        {
          "option": "Client-proposed numbers with server validation",
          "why_not": "Two clients placing concurrently would collide constantly and cancelled drafts would strand visible gaps; the server is the only authority that can be both monotonic and collision-safe."
        },
        {
          "option": "Live PATCH on every drag frame",
          "why_not": "Write amplification with no durability benefit: intermediate frames are transient by definition, and the one-natural-pixel contract only concerns the final position."
        },
        {
          "option": "Optimistically persist the new pin locally and reconcile later",
          "why_not": "An optimistic ghost has no server number; showing it would either fake a number or violate the monotonic-visible-numbers rule. The save latency on a local store is imperceptible."
        }
      ],
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "src/lib/server/annotations/pins.ts",
          "caption": "createPinAtomically / movePin: transactional numbering, idempotency, and revision rules."
        },
        {
          "type": "file",
          "path": "app/api/captures/[captureId]/annotations/route.ts",
          "caption": "List and create routes inside the established request boundary order."
        },
        {
          "type": "file",
          "path": "e2e/pins.spec.ts",
          "caption": "Persist/reload/isolation, monotonic numbering with a cancelled draft, camera zero-writes, and one-write drag commits at 1x and 8x."
        }
      ],
      "supersedes": null,
      "superseded_by": null,
    ...
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "Navigate mode never moves a mark and a tap on a saved pin selects it; pin dragging lives in Place pin mode, and e2e specs share the seeded plane by horizontal bands",
      "origin": "agent-autonomous",
      "status": "accepted",
      "problem": "Once persisted pins existed on the seeded capture, the full parallel e2e suite exposed three interaction holes: a pin-mode tap on an existing pin both selected it AND stacked a hidden draft on it; a camera-spec pan that happened to press a pin badge dragged the pin and committed a real PATCH during what the user meant as navigation; and three specs independently aiming their taps at the default pane center collided with each other's pins (a badge's 24px hit box spans hundreds of natural pixels at overview zoom, so per-point margins cannot keep taps clear).",
      "decision": "Persisted pins are draggable only in Place pin mode: in Navigate mode a press anywhere — including on a badge — pans, and camera work can never produce an annotation write. A pin-mode tap on a saved pin is selection, never placement. Draft pins stay draggable in any mode (adjusting a draft after switching back is the shipped flow) because drafts are transient and write nothing. In the e2e suite, gesture specs aim inside a middle document band while the pins spec writes into a bottom band via a shared lattice helper (findClearAim), making cross-spec collision structurally impossible rather than margin-unlikely.",
      "alternatives": [
        {
          "option": "Keep pins draggable in Navigate mode and make the camera spec avoid badges",
          "why_not": "That codifies the real defect: an accidental press-and-drag during navigation commits a durable move the user never intended. The interaction contract says navigation moves no mark."
        },
        {
          "option": "Give every e2e spec its own scratch capture",
          "why_not": "Scratch captures cost real provider work per run and lose the seeded deep-zoom document the canvas specs measure against; band separation keeps the shared plane usable."
        },
        {
          "option": "Clear pins from the store between e2e runs",
          "why_not": "There is no delete route by design (D059), and test-only database surgery would bypass the API contract the suite exists to prove."
        }
      ],
      "rationale": "Measured end-to-end failures forced each half: the mode gating and tap-selects rule resolve real write-on-pan and draft-stacking behavior, and band separation is the only aim strategy that survives the hit box's natural-pixel size at overview zoom. Both are inside the already-approved canvas direction (D051, D055, D056) and change no shipped scope, so they were safe to decide unilaterally.",
      "consequences": [
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "src/components/capture-canvas.tsx",
          "caption": "Mode-gated pin dragging and tap-selects placement skip."
        },
        {
          "type": "file",
          "path": "e2e/canvas-session.ts",
          "caption": "findClearAim banded lattice and panUntilNaturalVisible shared helpers."
        }
      ],
      "supersedes": null,
      "superseded_by": null
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "src/lib/server/annotations/pins.ts",
          "caption": "Create with elementId decision and server-derived snapshot; updatePin/deletePin with revision-precondition conditional writes."
        },
        {
          "type": "file",
          "path": "src/lib/server/annotations/context.ts",
          "caption": "Deterministic nearby-candidate ranker and snapshot derivation over the persisted manifest."
        },
        {
          "type": "file",
          "path": "app/api/captures/[captureId]/context/route.ts",
          "caption": "Authorized ready-capture-only context read route."
        },
        {
          "type": "file",
          "path": "src/components/capture-panel.tsx",
          "caption": "Draft decision fieldset, snapshot display, edit and two-step delete flows with conflict states."
        },
        {
          "type": "file",
          "path": "e2e/pin-lifecycle.spec.ts",
          "caption": "Browser contract for decision-required saves, revisioned mutations, staged two-session conflicts, and lost authority."
        }
      ],
      "supersedes": null,
      "superseded_by": null,
    ...
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "Anchor node drags at pointer-down: React Flow nodeDragThreshold set to 0 after e2e caught every drop landing a few pixels short",
      "origin": "agent-autonomous",
      "status": "accepted",
      "problem": "The pin-lifecycle e2e measured a dragged pin landing exactly one pointermove step short of the drop point (3 screen px in a 50px drag) on every run. Reading the installed @xyflow/system source showed why: with the default nodeDragThreshold of 1, startDrag captures the drag origin at the first pointermove past the threshold, so the initial travel is never applied to the node — a systematic drop error, not jitter.",
      "decision": "The canvas sets nodeDragThreshold={0} so drags anchor at pointer-down and the full pointer delta is applied. Click-versus-drag disambiguation stays with d3-drag's clickDistance, which already governs selection clicks, so tap-to-select is unchanged.",
      "alternatives": [
        {
        ...
        },
        {
          ...
          "why_not": "The swallowed amount depends on pointer speed and event coalescing, so it cannot be reconstructed; anchoring at pointer-down removes the error class entirely."
        }
      ],
      "rationale": "The failure was measured end-to-end and the library source confirmed the mechanism; the fix is one prop inside the already-approved React Flow canvas, so it was safe to decide unilaterally. jsdom cannot run d3-drag, so the e2e drag assertions are the regression guard.",
      "consequences": [
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "src/components/capture-canvas.tsx",
          "caption": "nodeDragThreshold={0} with the rationale comment."
        },
        {
          "type": "file",
          "path": "e2e/pin-lifecycle.spec.ts",
          "caption": "The failing measurement that caught the swallowed initial travel."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "validate",
      "title": "Fix the tall-motion-v1 focus flake in place: wire interaction counters before the scripted caret focus and exclude that focus by target",
      "origin": "agent-autonomous",
      "status": "accepted",
      "problem": "The real-provider motion suite intermittently failed its zero-interaction-counters assertion: tall-motion-v1 focused #caret-box before wiring its interaction counters, and Chromium defers focusin delivery for a page that is not focused yet, so the fixture's own scripted focus could land after the listeners were attached and be counted as an interaction (the known flake from user-testing round 1).",
      "decision": "Edit tall-motion-v1 in place rather than publishing a v2: the interaction counters are wired before the #caret-box focus call, and the focusin listener ignores events targeted at the fixture's own #caret-box, so the fixture's scripted focus is excluded deterministically no matter when Chromium delivers the event. Republished through npm run fixtures:publish, which re-verified a byte-exact readback and updated host.json.",
      "alternatives": [
        {
          "option": "Only reorder — wire the counters first, focus second, no exclusion",
          "why_not": "Ordering alone cannot remove the race: a deferred focusin can arrive at any later moment, and when delivery is synchronous the fixture's own focus would be counted deterministically. The assertion would fail always instead of intermittently."
        },
        {
          "option": "Publish the fix as a new tall-motion-v2 version",
          "why_not": "The change is render-invisible — identical layout, text, and pixels — so a new version would churn the publish script, host.json, and suite expectations for no capture-behavior difference. The versioning rule exists to protect the pixel-diff reference, which this change cannot move."
        },
        {
          "option": "Drop the scripted focus (and the caret motion case)",
          "why_not": "The caret case needs a real focused editable region to prove capture hides carets; removing it weakens the motion matrix the contract names."
        }
      ],
      "rationale": "The mission feature fixture-tall-motion-focus-ordering, created from the user-testing round 1 handoff, directed the in-place ordering fix; the exclusion-by-target is the smallest mechanism that makes the counter deterministic under deferred event delivery. Safe to decide unilaterally: it is a fixture-only change with no product surface, and the real-provider suite proves it three consecutive runs.",
      "consequences": [
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "test/fixtures/capture/tall-motion-v1.html",
          "caption": "Counter wiring now precedes the scripted caret focus; the focusin listener excludes the fixture's own caret-box focus."
        },
        {
          "type": "file",
          "path": "test/fixtures/capture/host.json",
          "caption": "Republished durable fixture record with the new tall-motion-v1 sha256."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    ...
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "The root route is a branded landing page: the pinata mark directly above the URL capture entry, a brief value proposition, a fully static example of a marked-up capture, and a clear sign-in path",
      "origin": "user-directed",
      "status": "accepted",
      "problem": "The root route was a bare product blurb plus a password prompt. An anonymous visitor could not tell what Pinata does, and the URL capture entry — the product's front door — only existed behind sign-in. The user asked for a real landing page, deliberately saved for the end of the build and kept (small, already specified) through the D051 descope.",
      "decision": "Rebuild / as the branded landing page for every visitor. The pinata mark — an accent tile carrying the product's own pin teardrop with a starburst, drawn once in src/lib/brand-mark.ts — renders inline above the capture entry and doubles as the favicon (app/icon.svg), with zero external image assets. The entry form takes a required root URL plus optional additional-URL rows with one visible primary action. Below the hero, a fully self-contained static example render (fixture data in src/lib/example-capture.ts) shows a screenshot region with two numbered pins, a two-entry comment thread, and a DOM metadata panel; it makes no /api/* request and no database access, and it depicts saved thread content only — no founder reply UI (D051). Anonymous visitors additionally get the editor sign-in prompt; a signed-in editor lands on the same branded page with the project form active, the project list below it, and the example after that.",
      "alternatives": [
        {
          "option": "A separate marketing page at / with the editor home at a different route",
          "why_not": "The user asked for the URL capture entry on the root page itself; splitting routes adds navigation the demo does not need."
        },
        {
          "option": "Render the example from a real recent capture in the database",
          "why_not": "The anonymous surface must not read project data, and the contract requires the example to trigger no /api/* request and no database access; bundled fixture data is the honest static answer."
        },
        {
          "option": "A raster or externally hosted logo image",
          "why_not": "Zero external assets is a stated requirement; one shared inline-SVG source for logo and favicon also keeps the two marks from drifting apart (locked by test/brand-mark.test.tsx)."
        }
      ],
      "rationale": "This is exactly what the user directed on 2026-09-08 and confirmed keeping on 2026-09-09; the implementation mechanics inside that direction are D067.",
      "consequences": [
      ...
      "transcript": {
        "request": "make sure to create a root page where the URL capture is entered, with a pinata logo above the form field. we need a decent landing page. maybe even render an example of a marked up page with comments and DOM metadata clear"
      },
      "artifacts": [
        {
          "type": "screenshot",
          "path": "screenshots/D066-landing.png",
          "caption": "The anonymous landing at 1440px: mark above the capture entry, value proposition, static example with two numbered pins, comment thread, and DOM metadata panel, then the sign-in prompt."
        },
        {
          "type": "file",
          "path": "src/lib/brand-mark.ts",
          "caption": "The single brand-mark source shared by the logo and the favicon."
        },
        {
          "type": "file",
          "path": "src/components/example-capture.tsx",
          "caption": "The fully static example render; the suite asserts it carries no client runtime or fetch."
        }
      ],
      "supersedes": null,
      "superseded_by": null,
    ...
    },
    {
      ...
      "alternatives": [
        {
          "option": "A dedicated /login route carrying the draft in query parameters",
          "why_not": "User-typed URLs would land in the address bar, history, and logs, and a new route duplicates the existing on-page prompt; the contract's flow is same-page before and after sign-in."
        },
        {
          "option": "localStorage instead of sessionStorage",
          "why_not": "The handoff is same-tab by construction (sign-in happens in the tab that submitted); localStorage would resurrect stale drafts in later sessions and other tabs."
        },
        {
          "option": "POST the draft to the server before authentication and reconcile after login",
          "why_not": "Anonymous project writes are unauthorized by design; the authorization boundary must not gain a pre-auth staging write for a cosmetic convenience."
        }
      ],
      "rationale": "Mechanical plumbing inside the user-directed landing direction (D066): it changes no product direction, touches no authorization rule, and the failure mode (storage unavailable or a malformed entry) degrades to a blank form. The consumed-once read prevents a reload from resurrecting a stale draft; malformed entries are validated and discarded, never thrown.",
      "consequences": [
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "src/lib/capture-draft.ts",
          "caption": "The validated save/take-once draft handoff."
        },
        {
          "type": "file",
          "path": "e2e/landing.spec.ts",
          "caption": "The end-to-end proof: no anonymous write, retained draft after sign-in, exactly one POST /api/projects answered 201."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "Deploy to Vercel production behind SSO protection, fixing the framework preset and adding a Protection-Bypass-for-Automation secret for the smoke",
      "origin": "agent-autonomous",
      "status": "accepted",
      "problem": "The first production deployment (D051) had to satisfy two hard constraints at once: deployment protection stays ON for the pinata project, and the production smoke (editor login, real Chickpea capture, private asset denial, pin persistence, /reqs hub) must run against the real deployment. The project was also still on the \"Other\" framework preset from before the Next.js stack landed, so the GitHub-triggered production builds of the validated commit were failing with Error status.",
      "decision": "Set the project framework preset to Next.js via the Vercel API, deployed the exact validated commit 523dcd9 with `vercel deploy --prod` (the CLI attaches the local git metadata, so VERCEL_GIT_COMMIT_SHA and the deployment record both carry the commit SHA the /reqs pages display), and enabled Vercel Protection Bypass for Automation: one high-entropy project secret, marked as the VERCEL_AUTOMATION_BYPASS_SECRET source, sent only as the x-vercel-protection-bypass header by the smoke tooling. SSO protection (all_except_custom_domains) is unchanged; the secret lives in Vercel and a local 0600 scratch file, never in the repository, logs, or .env.local.",
      "alternatives": [
        {
          "option": "Disable deployment protection for the smoke window, then re-enable it",
          "why_not": "The feature requires protection kept ON; a window with protection off is exactly the weakening the mission forbids, and the unprotected interval would be observable."
        },
        {
          "option": "Run the production smoke against the local production build instead (the milestone-1 substitution)",
          "why_not": "The substitution rule expired with milestone 2: this feature exists precisely to prove the real deployment — real Vercel runtime, real env configuration, real protection posture."
        },
        {
          "option": "Authenticate the smoke browser through Vercel SSO as the user",
          "why_not": "The user\"s Vercel account session is off-limits to agents; no headless credential path exists, and asking for an interactive login defeats automated re-verification."
        }
      ],
      "rationale": "Protection Bypass for Automation is Vercel\"s documented mechanism for exactly this situation: it keeps the SSO wall up for every party without the secret while letting automation through with a revocable, rotatable credential. Both hard constraints hold at once. The choice is mechanical execution inside the already-directed deployment scope (D051, and the readiness note that the deployment feature must fix the framework preset), not a product-direction change, so it was safe to decide without a round trip.",
      "consequences": [
      ...
      "artifacts": [
        {
          ...
          "caption": "The executable production smoke: protection posture, /reqs SHA identity, login, Chickpea capture drive, asset authorization, pin persistence, recapture isolation."
        },
        {
          "type": "file",
          "path": "e2e/production-smoke.spec.ts",
          "caption": "The browser-faithful production loop: UI sign-in, 8x dense-cell pin with explicit element choice, reload persistence, mobile plane isolation, unauthorized denial, SSO-wall check."
        },
        {
          "type": "file",
          "path": "scripts/chickpea-baseline.mjs",
          "caption": "The same-run direct-browser Chickpea baseline the capture landmarks are checked against (VAL-CAPTURE-011)."
        }
      ],
      "supersedes": null,
      "superseded_by": null,
    ...
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "Split the public landing from the editor workspace: / stays marketing, /pins is the app, /pins/new holds the project form",
      "origin": "user-directed",
      "status": "accepted",
      ...
      "decision": "Three routes with one job each. `/` is the public landing only — hero, anonymous capture entry, static example, sign-in, and five separate hub links generated from REQUIREMENTS_NAV. A verified editor session at `/` is redirected to `/pins`. `/pins` is the working surface: a compact header (home, New project, Sign out), the project workspace, and nothing that competes with the canvas. `/pins/new` carries the project form on its own route and hands off to `/pins` once the project commits. Both editor routes redirect an unverified visitor back to `/`, and all three share one server-side predicate (src/lib/server/auth/editor-page.ts) so the boundary cannot drift between them. Sign-in navigates rather than re-rendering in place, and routes to `/pins/new` when a parked anonymous draft (D067) is waiting so that handoff still works.",
      "alternatives": [
        {
          "option": "Keep one route and hide the hero once projects exist",
          "why_not": "The surface would still be one component deciding what it is at render time, and the project form would have no address of its own — there would be nothing for a 'New project' link to point at."
        },
        {
          "option": "Keep the project form permanently mounted above the workspace on /pins",
          "why_not": "That is the cost the split exists to remove: the form is used once per project and occupies the space the canvas needs on every visit."
        },
        {
          "option": "Make the five hub titles anchor links into sections of one /reqs page",
          "why_not": "The five routes already exist and already render from separate repository sources; pointing the titles at them is both less work and the behavior the titles already promise."
        }
      ],
      ...
      "transcript": {
        "request": "\"Requirements, architecture, milestones, decisions, and evals\" <-- these all point ot the same page, rather than the same page then the jump to the anchor point. i.e. clicking architecture goes to ./reqs/ rather than ./reqs/architecture/.\n\nUI changes:\n1. (image 1) only show the new project entry when the user is at root, otherwise have a \"new project\" link that directs back to this view. more space efficient. move the \"see what a marked-up capture\" (image 2) to the root as well, don't show when in the primary app.\n2. the changes in #1 above suggest we need a root route and a ./projects endpoint (or ./pins) endpoint for the primary app itself.",
        "approval": "/pins ... Redirect straight to the app route"
      },
      "artifacts": [
        {
          "type": "file",
          "path": "app/page.tsx",
          "caption": "The public landing, and the redirect that sends a verified editor to /pins."
        },
        {
          "type": "file",
          "path": "app/pins/page.tsx",
          "caption": "The editor workspace route, gated by redirect."
        },
        {
          "type": "file",
          "path": "app/pins/new/page.tsx",
          "caption": "The project form on its own route."
        },
        {
          "type": "file",
          "path": "src/lib/server/auth/editor-page.ts",
          "caption": "The single shared editor-session predicate the three routes agree on."
        },
        {
          "type": "file",
          "path": "src/components/landing.tsx",
          "caption": "LandingLinks: five links from REQUIREMENTS_NAV instead of one anchor."
        }
      ],
      "supersedes": null,
      "superseded_by": null,
    ...
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "Collapse the project rail into nested native disclosures, open only around the current selection",
      "origin": "user-directed",
      "status": "accepted",
      "problem": "The left rail printed every project's entire page and device tree at once. With more than a couple of projects the canvas was pushed off screen, and the rail gave no way to put a project away once its captures were reviewed.",
      "decision": "Two levels of native <details>: one around the whole rail labelled 'Projects' with a count, and one per project around its pages and devices. The rail starts open; a project starts open only when it holds the current selection, and stays wherever the reader last put it. The disclosure state is session-only React state, never persisted. Each project keeps its heading for assistive technology, now visually hidden inside the disclosure while the summary carries the visible title.",
      "alternatives": [
        {
          "option": "A hand-rolled button with aria-expanded and a controlled region",
          "why_not": "More code and more ways to get the announcement wrong, for behavior <details> already provides correctly and without script."
        },
        {
          "option": "Default every project collapsed",
          "why_not": "The canvas is showing something; collapsing the control that produced it hides the reader's own context."
        },
        {
          "option": "Default every project expanded, with collapse available",
          "why_not": "That is the current behavior plus a control nobody has a reason to press; it does not recover the space the change exists to recover."
        }
      ],
      "rationale": "Native disclosures are keyboard-operable and correctly announced with no dependency and no script, which matches the repository's standing constraint. Anchoring the default to the selection means the rail is never hiding the thing the reader is looking at, while every other project folds away.",
      "consequences": [
      ...
      "transcript": {
        "request": "3. in the primary app, set it so that Projects in the left-hand rail are moved inside of a collapsable/expandable nav element to save space."
      },
      "artifacts": [
        {
          "type": "file",
          "path": "src/components/project-workspace.tsx",
          "caption": "The nested disclosures and the selection-anchored default."
        },
        {
          "type": "file",
          "path": "e2e/canvas-session.ts",
          "caption": "revealPlane/clickPlane: the specs expand a collapsed project before selecting a plane."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "List every pin in a table below the canvas, with a Markdown export for pasting into an agentic IDE",
      "origin": "user-directed",
      "status": "accepted",
      "problem": "The side panel shows one pin at a time. That is right for editing, and useless for the thing the pins are ultimately for: handing a page's worth of feedback to a coding agent. Getting all of it out meant clicking each pin in turn and copying the comment by hand, losing the element context that makes a note actionable without the screenshot.",
      ...
      "alternatives": [
        {
          "option": "A per-row copy button instead of one copy-all",
          "why_not": "The user asked for the whole set in one paste; per-row copying is the manual work the table exists to replace."
        },
        {
          "option": "Export JSON",
          "why_not": "The destination is a chat-style agent prompt, where prose with inline context reads better than a structure the agent has to interpret."
        },
        {
          "option": "Extend the side panel to list full pin detail",
          "why_not": "The panel is screen-fixed and narrow by design so it survives panning; a full table there would either overflow or force the canvas to shrink."
        }
      ],
      "rationale": "The element snapshot is the only durable record of where a note points — the capture is a screenshot and the manifest is never re-derived after save (VAL-PIN-003, VAL-PIN-008) — so an export that omits it is not actionable. Emitting the body inside a blockquote keeps arbitrary comment text intact without escaping the content that matters most.",
      "consequences": [
      ...
      "transcript": {
        "request": "Pin+annotation set as list view - at the bottom of each project view, rather just one at a time in the right-hand pin manipulation component, have an array of pins - pin #, pin location (from attached DOM object), and copy. this should make it easier to copy past into an agentic IDE/dev tool.",
        "approval": "Table + \"Copy all as Markdown\" button"
      },
      "artifacts": [
        {
          "type": "file",
          "path": "src/lib/pin-export.ts",
          "caption": "The pure Markdown rendering the clipboard receives."
        },
        {
          "type": "file",
          "path": "src/components/pin-table.tsx",
          "caption": "The all-pins table and the copy control."
        },
        {
          "type": "file",
          "path": "test/pin-export.test.ts",
          "caption": "The export contract: element context present, comment bodies verbatim."
        }
      ],
      "supersedes": null,
      "superseded_by": null,
    ...
    },
    {
      ...
      "alternatives": [
        {
          "option": "A static HTML or Markdown slide deck",
          "why_not": "The user asked for Remotion specifically, and a static deck cannot show the pin dropping, the canvas zooming with the pins staying put, or the thread growing, which are the parts of the product that prose explains worst."
        },
        {
          "option": "A screen recording of the live app",
          "why_not": "A recording goes stale the moment the UI changes and cannot be regenerated from source; a composition re-renders from the same code the tests cover."
        },
        {
          "option": "More than ten slides, one per feature",
          "why_not": "The user sized it at about ten, and ten is what a viewer will sit through before the live demo; the /reqs hub already holds the long form."
        }
      ],
      "rationale": "Directed by the user. Remotion keeps the walkthrough in code, typed by the same tsconfig as the app and covered by the same gate, and its Player lets the same composition run in the deployed product, so the walkthrough is demoable over a link rather than only from a checkout. Borrowing the committed board and screenshots keeps the visual language honest: it is the product's own material, not stock.",
      "consequences": [
      ...
      "transcript": {
        "request": "use the remotion library and create an interactive walkthrough on what pinata is and how it works. think maybe 10 total slides and borrow from any imagery that exists in the repository"
      },
      "artifacts": [
        {
          "type": "file",
          "path": "remotion/walkthrough/slides.ts",
          "caption": "The ten-slide table: chapters, durations, transcript."
        },
        {
          "type": "file",
          "path": "remotion/walkthrough/Walkthrough.tsx",
          "caption": "The composition, one component per slide id."
        },
        {
          "type": "file",
          "path": "public/walkthrough/brand-board.webp",
          "caption": "The borrowed brand exploration board the title slide draws its tiles from."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "src/components/walkthrough-player.tsx",
          "caption": "The in-app player with chapter navigation."
        },
        {
          "type": "file",
          "path": "scripts/lib/approved-deps.mjs",
          "caption": "The allowlist, now naming the three Remotion packages."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    ...
    },
    {
      ...
      "transcript": {
        "request": "go ahead and fix B1, then merge. and tackle each of your concerns thereafter."
      },
      "artifacts": [
        {
          "type": "file",
          "path": "src/lib/server/projects/hierarchy.ts",
          "caption": "Header total summed over selected captures."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    ...
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "Build the founder link and read/reply loop after all, on a parallel branch, partly un-deferring D051",
      "origin": "user-directed",
      "status": "accepted",
      "key": true,
      ...
      "alternatives": [
        {
          "option": "Keep it deferred and ship the editor-only build",
          "why_not": "The founder's reply is the product's reason to exist; leaving it out makes the demo a screenshot annotator."
        },
        {
          "option": "Build it directly on main alongside the demo work",
          "why_not": "It touches auth, the asset route, and the canvas. A half-finished founder session on main could break a working demo the day it is shown."
        },
        {
          "option": "Wait until after the live pins checkpoint",
          "why_not": "The work is independent of the canvas and pin code the checkpoint exercises, so serializing it only spends calendar time."
        }
      ],
      "rationale": "The human directed the stream, its priority relative to the demo, and the branch shape. This is a bounded exception to D010's commit-straight-to-main, not a reversal of it: main keeps the gate before every commit, and the branch carries the same gate.",
      "consequences": [
      ...
      "transcript": {
        "request": "let's do this work (below) in parallel, as it's not crucual to the demo, but will make it more usable for future use in the wild with friends. open a separate branch for this and we'll own a PR later as well. I don't want to block/break what's already working."
      },
      "artifacts": [
        {
          "type": "file",
          "path": "src/components/founder-view.tsx",
          "caption": "The founder's read/reply-only shell."
        },
        {
          "type": "file",
          "path": "src/lib/server/founder/capability.ts",
          "caption": "Capability issue, rotate, and revoke."
        },
        {
          "type": "file",
          "path": "e2e/founder.spec.ts",
          "caption": "Environment-gated end-to-end proof of the link and reply loop."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "Founder capability sessions are stateless: a signed cookie bound to project id and capability version, re-checked against the project row on every request",
      "origin": "agent-autonomous",
      "status": "accepted",
      "problem": "A founder link must persist until rotated or revoked, and rotation or revocation must end every existing founder session at once. The editor session revokes through an in-memory denylist (a documented per-instance weakness); copying that for founders would let a rotated capability keep working on another serverless instance until absolute expiry.",
      "decision": "Mirror the editor session code for signing, lifetime, renewal, cookie attributes, and the double-submit CSRF proof, but bind the founder payload to {project id, capability version} and have the guard re-read the project row on every request: the project must be live, the digest present, share_revoked_at null, and share_token_version equal to the session's version. No founder session state is persisted, so no migration is needed and no denylist exists to go stale.",
      "alternatives": [
        {
          "option": "A founder_sessions table with explicit revocation rows",
          "why_not": "A new table and migration for state the project row already expresses; every request would still have to read the project row to learn about rotation."
        },
        {
          "option": "Reuse the editor session token format with a role claim",
          "why_not": "One token format for two authorities invites confusion; a distinct prefix (f1 versus v1) covered by the signature makes cross-verification impossible by construction, and the tests prove it."
        }
      ],
      "rationale": "The brief explicitly says the version binding is what invalidates sessions, and the durable row is the only authority that changes on rotation; reading it per request is one indexed primary-key lookup. Safe to decide alone because it adds no schema, no dependency, and no new boundary constant.",
      "consequences": [
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "src/lib/server/founder/session.ts",
          "caption": "Signed founder session bound to project and version."
        },
        {
          "type": "file",
          "path": "src/lib/server/founder/guard.ts",
          "caption": "Live binding check on every request."
        },
        {
          "type": "file",
          "path": "test/server/founder-routes.test.ts",
          "caption": "Rotation and revocation end existing sessions."
        }
      ],
      "supersedes": null,
      "superseded_by": null,
    ...
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "No new boundary constants for founder links: reuse the editor session policy, AUTH_REQUEST_MAX_BYTES, ANNOTATION_REQUEST_MAX_BYTES, FEEDBACK_BODY_MAX_CHARS, and the published reply quota",
      "origin": "agent-autonomous",
      "status": "accepted",
      "problem": "The boundary catalog is versioned and drift-checked against docs/EVALS.md and docs/ARCHITECTURE.md; adding any constant requires a POLICY_VERSION bump published in both documents, and this branch is forbidden from editing ARCHITECTURE.md while another stream closes the docs out.",
      "decision": "Every limit the founder and thread surfaces need already exists in the catalog: the exchange body is capped by AUTH_REQUEST_MAX_BYTES (a 43-character token fits comfortably), reply bodies by ANNOTATION_REQUEST_MAX_BYTES and FEEDBACK_BODY_MAX_CHARS, founder session lifetime by the editor session constants, and the founder reply quota by REPLY_MAX_PER_WINDOW / REPLY_WINDOW_MS. The token length itself (32 bytes, 43 base64url characters) lives in the capability module as an implementation constant, not a published boundary, because it is a cryptographic minimum rather than a tunable policy.",
      "alternatives": [
        {
          "option": "Add FOUNDER_SESSION_* and FOUNDER_TOKEN_BITS to the catalog with a POLICY_VERSION bump",
          "why_not": "Forces edits to ARCHITECTURE.md and EVALS.md that this branch may not make, for values that would simply duplicate the editor's."
        }
      ],
      "rationale": "The brief allows EVALS.md edits only when a drift test forces them; reusing constants means no drift test fires and the catalog stays at 2026-09-09.2. If the owner later wants a shorter founder lifetime, that is a one-constant catalog change with its own decision record.",
      "consequences": [
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "src/lib/server/founder/capability.ts",
          "caption": "Token size as an implementation constant."
        },
        {
          "type": "file",
          "path": "src/lib/server/threads/throttle.ts",
          "caption": "Reply quota from the existing catalog values."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "src/lib/server/founder/reader.ts",
          "caption": "Editor-or-founder authorizer."
        },
        {
          "type": "file",
          "path": "test/server/capture-asset-founder.test.ts",
          "caption": "Founder bytes and the one denial shape."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "Thread replay is intent-bound without a digest column: the same key returns the committed entry only when body and role match, otherwise it conflicts",
      "origin": "agent-autonomous",
      "status": "accepted",
      "problem": "Thread appends must be idempotent per the existing (annotation_id, idempotency_key) unique index, and pin creation binds a key to a payload digest through the idempotency_keys table. thread_entries has no digest column and adding one means a migration for a table whose rows can never be updated.",
      "decision": "Look up the existing entry by (annotation, key); replay it with 200 when its body and actor role equal the request's, answer 409 when they differ, and converge on the winner if the unique index fires under concurrency. The founder quota admission runs only after the replay check and validation, so a retried or invalid reply never consumes quota.",
      "alternatives": [
        {
          "option": "Record thread appends in idempotency_keys with a payload digest",
          "why_not": "Two sources of truth for one row; the entry itself already carries every field the digest would cover."
        },
        {
          "option": "Add a payload_digest column to thread_entries",
          "why_not": "A migration for information derivable from the row."
        }
      ],
      "rationale": "The entry row is immutable, so comparing its stored body and role is exactly as strong as comparing a digest of them. Safe to decide alone: no schema change, and the behavior matches the pin store's replay-or-conflict contract.",
      "consequences": [
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "src/lib/server/threads/entries.ts",
          "caption": "Replay-or-conflict append."
        },
        {
          "type": "file",
          "path": "test/server/thread-entries.test.ts",
          "caption": "Replay, conflict, quota admission ordering, and the trigger proof."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "The editor's share control is closed by default and reads nothing until opened; the workspace's default traffic is unchanged",
      "origin": "agent-autonomous",
      "status": "accepted",
      "problem": "The workspace tests stub fetch and count requests; an always-on per-project status read would add a request per project on every mount and change the default behavior the brief says must stay unchanged.",
      "decision": "Render one Share with founder toggle per project in the tree; opening it reads the status once and offers Create/Rotate and Revoke; the issued link is composed client-side as origin + path + '#' + token, shown once in a read-only field with a Copy control, and forgotten when the control closes. The hierarchy payload is not extended with share status.",
      "alternatives": [
        {
          "option": "Include share status in the project hierarchy response",
          "why_not": "Changes the hierarchy shape every existing fixture constructs and adds capability metadata to a payload the founder route also serves."
        }
      ],
      "rationale": "Additive and minimal, as the brief asks; the only new default-path DOM is one button per project, and the existing workspace suite passes without modification.",
      "consequences": [
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "src/components/founder-share.tsx",
          "caption": "The share control."
        },
        {
          "type": "file",
          "path": "test/founder-share.test.tsx",
          "caption": "Closed by default, link shown once, rotate and revoke."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "build",
      "title": "Founder pages get their security headers from next.config.ts for /f/* and /api/founder/*: no referrer, no indexing, no framing, no caching; a full nonce-based CSP is deferred",
      "origin": "agent-autonomous",
      "status": "accepted",
      "problem": "Architecture requires founder capability pages to send no referrer and permit no indexing, and names a strict CSP with frame-ancestors 'none'. The application currently sets no response headers anywhere, and a strict script-src CSP would need per-request nonces threaded through the Next.js layout.",
      "decision": "Add a headers() block to next.config.ts that sets Referrer-Policy: no-referrer, X-Robots-Tag: noindex, nofollow, noarchive, X-Frame-Options: DENY, Content-Security-Policy: frame-ancestors 'none', and Cache-Control: private, no-store, max-age=0 on /f/:path* and /api/founder/:path*; the founder page also declares robots noindex and referrer no-referrer in its metadata. The token never appears in a path or query, so no header can leak it.",
      "alternatives": [
        {
          "option": "A full strict CSP with nonces for the founder page",
          "why_not": "Requires middleware-generated nonces and layout plumbing across the whole app; out of this feature's scope and not needed for the no-referrer / no-index requirement."
        }
      ],
      "rationale": "This satisfies the stated founder-page requirements with framework configuration only; the strict CSP remains a documented gap for a follow-up decision.",
      "consequences": [
      ...
      "artifacts": [
        {
          "type": "file",
          "path": "next.config.ts",
          "caption": "Founder surface headers."
        },
        {
          "type": "file",
          "path": "app/f/[publicId]/page.tsx",
          "caption": "Metadata: noindex, no-referrer."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "wrap",
      "title": "Disclose the tooling used after the Missions: Claude Code on mobile for the documentation closeout, and a second harness for the cross-review",
      "origin": "user-directed",
      "status": "accepted",
      "key": true,
      ...
      "alternatives": [
        ...
        },
        {
          ...
          "why_not": "The work is documentation. Redoing it buys a cleaner-looking story and nothing else."
        },
        {
      ...
      "artifacts": [
        ...
        },
        {
          ...
          "caption": "The time-and-tooling ledger at the top of the log."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "alternatives": [
        {
          "option": "Keep the curated list in the page component rather than in the data",
      ...
      "rationale": "The human directed the substance: the decision log reachable in the app, focused on the critical product and architecture calls. Where the flag lives — in the source data rather than a component — is mechanical inside that direction, and putting it in the data is what lets all three surfaces agree.",
      "consequences": [
      ...
      "transcript": {
        "request": "yes, let's have have the human-friendly decision log accessible from the app so it's all in one. also, focus on critical product or architecture decisions to surface in that context."
      },
      "artifacts": [
        {
          "type": "file",
          "path": "scripts/lib/decisions.mjs",
          "caption": "Validator and generator support for the key flag."
        },
        {
          "type": "file",
          "path": "src/components/decisions-catalog.tsx",
          "caption": "Key-decisions index above the full catalog."
        },
        {
          "type": "file",
          "path": "docs/dashboard/index.html",
          "caption": "Key-only filter chip and per-record pill."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "date": "2026-09-10",
      "phase": "wrap",
      "title": "Cross-review the repository with a second agent harness and adopt its findings selectively, rather than merging its branch",
      "origin": "user-directed",
      "status": "accepted",
      "key": true,
      ...
      "alternatives": [
        {
          "option": "Merge the review branch as-is and fix the fallout afterwards",
          "why_not": "Three of its four records would have collided by id with main's own D069 through D071, and six documents would have re-asserted that the founder loop was unbuilt. Cheaper to take the findings than the commits."
        },
        {
          "option": "Ignore the branch and keep a single harness",
          "why_not": "The branch found four real defects nothing in the gate catches, including the .gitignore ordering bug and the stale test reference in AGENTS.md. A second harness reading the whole repository cold is a cheap audit."
        },
        {
          "option": "Cherry-pick the branch's commits",
          "why_not": "Its four commits each mix adopted and rejected changes; picking them would need the same file-level resolution with less clarity about what was accepted and why."
        }
      ],
      "rationale": "The owner asked for the second harness specifically to hold the work doubly accountable, so its findings deserved evaluation rather than either automatic merge or dismissal. Selecting file by file keeps the accepted changes attributable and leaves the rejections stated here instead of implied by a silent revert.",
      "consequences": [
      ...
      "transcript": {
        "request": "take a look at origin/claude/droid-project-review-2qv0o3 -- i wanted to hold us doubly accountable with another harness - and see which updates you agree with. make those changes in a branch here. then let's merge. after that we'll delete the claude-named branch."
      },
      "artifacts": [
        {
          "type": "file",
          "path": ".gitignore",
          "caption": "The negation moved below the broader .env* pattern that had been re-ignoring it."
        },
        {
          "type": "file",
          "path": ".env.example",
          "caption": "Environment names only, adopted from the cross-review."
        },
        {
          "type": "file",
          "path": "AGENTS.md",
          "caption": "Section 5 layout tree and the corrected test references."
        }
      ],
      "supersedes": null,
      "superseded_by": null
      ...
      "alternatives": [
        {
          ...
          "why_not": "A build that migrates the shared production database on every preview deployment is worse than a manual step in the right order."
        },
        {
          "option": "Turn deployment protection off",
          "why_not": "Would expose every preview deployment, and their per-commit URLs, as well as production."
        },
        {
    ...
    },
    {
      ...
      "transcript": {
        "request": "use the Vercel CLI (if possible) to use the new yourpinata.dev domain I purchased with Vercel Domains / cool. we're local now. we have the vercel cli installed. run through all of the above."
      },
      "artifacts": [
        {
          "type": "link",
          "url": "https://yourpinata.dev",
          "caption": "Production, served from the custom domain."
        }
      ],
      "supersedes": null,
      "superseded_by": null
    },
    {
      ...
      "date": "2026-09-25",
      "phase": "validate",
      "title": "Raise the per-client editor login failure limit from 5 to 10, keeping the global backstop at 100",
      "origin": "user-directed",
      "status": "accepted",
      "problem": "On the first turn of the milestone-2/3 live checkpoint the owner was locked out of editor sign-in after five failed attempts and told to wait thirteen minutes. The failures were mechanical rather than hostile: the agent had copied EDITOR_PASSWORD out of .env.local with its surrounding double quotes still attached, so several attempts could not have succeeded. D098 had already replaced D026's single shared bucket with a per-client bucket plus a global backstop, which stops one noisy client locking the editor out — but the per-client key falls back to a fixed key when there is no x-real-ip or x-forwarded-for, which is exactly the local checkpoint case. So the limit that stopped the owner is still five, and a solo operator running the checkpoint on 127.0.0.1 meets it as easily as before.",
      "decision": "Raise LOGIN_MAX_FAILURES from 5 to 10. LOGIN_GLOBAL_MAX_FAILURES stays at 100, LOGIN_WINDOW_MS stays at 900,000 ms, and D098's reserve-then-verify ordering, two-bucket accounting, 429 answer, and success-clears-the-client-bucket behaviour are all untouched. POLICY_VERSION moves to 2026-09-25.1.",
      "alternatives": [
        {
          "option": "Remove the brute-force blocker entirely",
          "why_not": "What the owner first asked for. The agent raised the exposure — one shared password is the only control in front of the editor account on a live deployment — and the owner narrowed the request to the failure count in the same breath."
        },
        {
          "option": "Raise the per-client limit to 20 and the global ceiling to 200 to match",
          "why_not": "The agent tried exactly this first and the gate refused it: test/boundaries.test.ts asserts LOGIN_GLOBAL_MAX_FAILURES >= 10 * LOGIN_MAX_FAILURES so that one client can never exhaust the backstop. Keeping the ratio by doubling the global ceiling would have doubled the distributed-guessing budget to buy headroom nobody asked for. Ten is the largest per-client value the published invariant allows without touching the global ceiling."
        },
        {
          "option": "Leave it at five and clear the bucket whenever the checkpoint trips it",
          "why_not": "The agent had already done this once by hand. It costs nothing but leaves the defect in the product and makes the agent a required participant in the owner signing in."
        },
        {
          "option": "Give the local fixed-key fallback its own higher limit",
          "why_not": "It would put an environment-shaped exception inside a published boundary, and the published catalog is deliberately one value per policy. If local ergonomics and deployed exposure ever really diverge, that is a boundary change with its own record, not a hidden branch."
        }
      ],
      "rationale": "Ten per client per fifteen minutes is still a hard ceiling against an eleven-character secret, and the control that actually matters against a distributed guesser — the global backstop of 100 — does not move at all. What changes is only how quickly a single well-meaning operator can lock themselves out, which is the whole of the observed problem. Ten is also the largest value D098's published invariant permits while leaving the global ceiling alone, so the change takes the available headroom rather than inventing new exposure. Every test imports LOGIN_MAX_FAILURES rather than hardcoding five, so no assertion weakens.",
      "consequences": [
      ...
      "transcript": {
        "request": "raise the failure count. i'm not stressed here yet"
      },
      "artifacts": [],
      "supersedes": null,
      "superseded_by": null
